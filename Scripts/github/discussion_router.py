#!/usr/bin/env python3
"""Route GitHub Discussion events to repo-grounded response responders."""
...
REQUIRED_RESPONDER_IDS = ("technical", "support", "framework_governance")
DEFAULT_CONFIG_PATH = Path(".github") / "discussion-automation.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Respond to GitHub Discussion events using repo-grounded responders."
    )
    parser.add_argument("--root", default=os.getcwd(), help="Repository root path.")
    parser.add_argument(
        "--config",
        default=None,
        help="Optional path to the discussion responder config file.",
    )
    parser.add_argument(
        "--event-path",
    ...
    parser.add_argument(
        "--validate-config",
        action="store_true",
        help="Validate the discussion responder config and exit.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def validate_config(config: dict[str, Any], root: Path) -> list[str]:
    ...
    if config.get("version") != "1.0":
        errors.append("discussion-automation.json must declare version 1.0.")

    default_responder_id = config.get("default_responder_id")
    responders = config.get("responders")
    response_policy = config.get("response_policy") or {}

    if not isinstance(responders, list):
        errors.append("discussion-automation.json must define an responders list.")
        return errors
    ...
    if fallback_message != "There is not information available at this time. Check back soon.":
        errors.append(
            "discussion-automation.json must preserve the required fallback message."
        )

    seen_ids: set[str] = set()
    for responder in responders:
        if not isinstance(responder, dict):
            errors.append("Each discussion responder entry must be a JSON object.")
            continue

        responder_id = responder.get("id")
        if not responder_id:
            errors.append("Every discussion responder must define an id.")
            continue

        seen_ids.add(responder_id)

        if not responder.get("display_name"):
            errors.append(f"Discussion responder '{responder_id}' must define display_name.")

        category_hints = responder.get("category_hints")
        if not isinstance(category_hints, list) or not category_hints:
            errors.append(
                f"Discussion responder '{responder_id}' must define category_hints."
            )

        keywords = responder.get("keywords")
        if not isinstance(keywords, dict) or not keywords:
            errors.append(f"Discussion responder '{responder_id}' must define keywords.")

        source_paths = responder.get("source_paths")
        if not isinstance(source_paths, list) or not source_paths:
            errors.append(f"Discussion responder '{responder_id}' must define source_paths.")
            continue

        for source_path in source_paths:
            if not (root / source_path).exists():
                errors.append(
                    f"Discussion responder '{responder_id}' references a missing source path: {source_path}"
                )

    if tuple(sorted(seen_ids)) != tuple(sorted(REQUIRED_RESPONDER_IDS)):
        errors.append(
            "discussion-automation.json must define exactly the technical, support, and framework_governance responders."
        )

    if default_responder_id not in seen_ids:
        errors.append("discussion-automation.json default_responder_id must match an responder id.")

    max_sources = response_policy.get("max_sources")
    if not isinstance(max_sources, int) or max_sources < 1:
        errors.append("discussion-automation.json response_policy.max_sources must be >= 1.")

    max_excerpt_chars = response_policy.get("max_excerpt_chars")
    if not isinstance(max_excerpt_chars, int) or max_excerpt_chars < 120:
        errors.append(
            "discussion-automation.json response_policy.max_excerpt_chars must be >= 120."
        )
...
def score_responder(responder: dict[str, Any], category_name: str, query_text: str) -> int:
    query_lower = normalize_text(query_text)
    category_lower = normalize_text(category_name)
    query_tokens = tokenize(query_text)
    score = 0

    for hint in responder.get("category_hints", []):
    ...
    for phrase, weight in (responder.get("keywords") or {}).items():
...
def choose_responder(
    ...
    for responder in config["responders"]:
        results = search_sources(
            root,
            responder["source_paths"],
            query_text,
            max_sources=max_sources,
            max_chars=max_chars,
        )
        retrieval_score = results[0]["score"] if results else 0
        classification_score = score_responder(responder, category_name, query_text)
        total_score = classification_score + retrieval_score
        ranked.append((total_score, classification_score, retrieval_score, responder, results))

    ranked.sort(key=lambda item: (-item[0], -item[2], item[3]["id"]))

    if ranked and ranked[0][0] > 0:
        _, classification_score, retrieval_score, responder, results = ranked[0]
        if classification_score == 0 and retrieval_score < 8:
            break_glass_responder_id = config["default_responder_id"]
            for _, _, _, fallback_responder, _ in ranked:
                if fallback_responder["id"] == break_glass_responder_id:
                    return fallback_responder, []
            return responder, []
        return responder, results

    default_responder_id = config["default_responder_id"]
    for _, _, _, responder, results in ranked:
        if responder["id"] == default_responder_id:
            return responder, results

    raise RuntimeError("No default discussion responder could be selected.")


def render_response(
    responder: dict[str, Any], results: list[dict[str, Any]], fallback_message: str
) -> str:
    lines = [
...
def post_discussion_comment(
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "forsetti-framework-windows-discussion-automation",
        },
        method="POST",
    )
...
def main() -> int:
    ...
    if not config_path.is_file():
        print(f"Missing discussion responder config: {config_path}", file=sys.stderr)
        return 1
    ...
    if args.validate_config:
        print("Discussion responder configuration is valid.")
        return 0
    ...
    responder, results = choose_responder(
        config,
        root,
    ...
    print(
        f"Posted {responder['display_name']} response for discussion event from {context['sender_login']}."
    )
    return 0
