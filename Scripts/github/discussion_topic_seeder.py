...
import discussion_router

TEXT_ENCODING = "utf-8-sig"
DEFAULT_CONFIG_PATH = Path(".github") / "discussion_topic_generator.json"
DEFAULT_DISCUSSION_ROUTER_CONFIG_PATH = Path(".github") / "discussion-automation.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--discussion-router-config",
        default=None,
        help="Optional path to the discussion response responder config file.",
    )
    parser.add_argument(
        "--validate-config",
        action="store_true",
...
def slugify(value: str) -> str:
    normalized = discussion_router.normalize_text(value)
...
def classify_family(
    discussion_router_config: dict[str, Any], title: str, summary: str, source_path: str
) -> str:
    query_text = "\n".join(part for part in (title, summary, source_path) if part)
    ranked: list[tuple[int, str]] = []
    for responder in discussion_router_config["responders"]:
        score = discussion_router.score_responder(responder, "", query_text)
        ranked.append((score, responder["id"]))

    ranked.sort(key=lambda item: (-item[0], item[1]))
    if ranked and ranked[0][0] > 0:
        return ranked[0][1]
    return discussion_router_config["default_responder_id"]


def build_markdown_heading_candidates(
    root: Path,
    source: dict[str, Any],
    discussion_router_config: dict[str, Any],
) -> list[dict[str, Any]]:
...
def build_topic_candidates(
    ...
    for source in config["seed_sources"]:
        if source["kind"] == "markdown_headings":
            candidates.extend(
                build_markdown_heading_candidates(root, source, discussion_router_config)
            )
        elif source["kind"] == "wiki_sync_pages":
            candidates.extend(
                build_wiki_sync_candidates(root, source, discussion_router_config)
            )
...
def graphql_request(
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "forsetti-framework-windows-discussion-topic-seeder",
        },
        method="POST",
    )
...
def choose_category(
    ...
    normalized = {
        category["id"]: {
            **category,
            "slug_normalized": discussion_router.normalize_text(category["slug"]),
            "name_normalized": discussion_router.normalize_text(category["name"]),
        }
        for category in categories
    }

    for preferred in category_preferences.get(family, []):
        preferred_normalized = discussion_router.normalize_text(preferred)
...
def discussion_exists(
    ...
    normalized_title = discussion_router.normalize_text(candidate["title"])
    for discussion in existing_discussions:
        title = discussion_router.normalize_text(discussion.get("title") or "")
...
def main() -> int:
    ...
    discussion_router_config_path = (
        Path(args.discussion_router_config)
        if args.discussion_router_config
        else root / DEFAULT_DISCUSSION_ROUTER_CONFIG_PATH
    )

    if not config_path.is_file():
        print(f"Missing discussion topic generator config: {config_path}", file=sys.stderr)
        return 1
    if not discussion_router_config_path.is_file():
        print(
            f"Missing discussion responder config required for topic classification: {discussion_router_config_path}",
            file=sys.stderr,
        )
        return 1
    ...
    discussion_router_config = load_json_file(discussion_router_config_path)
    discussion_router_errors = discussion_router.validate_config(
        discussion_router_config, root
    )
    if discussion_router_errors:
        for error in discussion_router_errors:
            print(
                f"ERROR: discussion_topic_responder depends on a valid discussion responder config. {error}",
                file=sys.stderr,
            )
        return 1
    ...
    candidates = build_topic_candidates(root, config, discussion_router_config)
    ...
    graphql_url = discussion_router.derive_graphql_url(
